...
19. 시뭔스에 의해 지동으로 생성되는 가상의 열 > 자동 증가 열
20. CREATE VIEW 명령을 이용해서 기몬 테이블로부터 만들어지는 테이블 > View
22. 단 하나의 테이블만 기초로 하여 생성된 뷰 > 단일 뷰
...
5. 02번 문제에서 생성한 lab 테이블의 ‘호실 (rooffi_id) ’ 열에 rooffi_id_idx라는 인텍스를 생성하는 SQL 문을 작성하시오.
CREATE INDEX room_id_idx
ON lab(room_id);

7. 02번 문제에서 생성한 1ab 태이블에 다음 데이터를 삽입하는 SQL 문을 작성하시오.
INSERT INTO lab
VALUES (
188,
'가상현실',
